...
class Game
  ...
  def initialize
    puts "Please enter a dimension for your board:"
  ...
  def run
    lose = 823475
    num_bombs = @b.seed
    num_of_spaces = (@dimension ** 2) - num_bombs
    until lose == nil
      revealed_squares = 0
      @b.render
  ...
  def cursor(move)
    ...
    if (@selector.first + move.first).between?(0,(@dimension - 1))
      @selector[0] += move.first
    end
    if (@selector.last + move.last).between?(0,(@dimension - 1))
...
class Tile
  ...
  def initialize(board,position)
    @revealed = nil
    @board = board
    ###NEED TO DEFINE DIMENSION IN THIS CLASS
    @position = position
    @selected = :selected
  end
    ...
    for x in (-1..1)
      for y in (-1..1)
        ...
        unless @board[[x,y]] == self
          if x_neighbor.between?(0, (@board.dimension - 1)) && y_neighbor.between?(0, (@board.dimension - 1))
